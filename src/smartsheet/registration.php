<?php
    ...
    $sheetId = '8947603246212996';
    $apiKey = 'fvosg6gvsqla2wgxv87jku09ao';
    $clinicField = 'VOC dantų laminatės landing';
    ...
    function createData($response){
        ...
        foreach($response as $item)
        {
            ...
            if($item->type === 'TEXT_NUMBER')
            {
                ...
                // }
                if($item_title === 'Column2') {
                    $column_id = $item->id;
                    $column_value =  $clinicField;
                } else if($item_title === 'Column3')  {
                    $column_id = $item->id;
                    $column_value =  $inputs->name;
                } else if($item_title === 'Column4') {
                        $column_id = $item->id;
                    $column_value =  $inputs->phone;
                } 
                if($column_id)
